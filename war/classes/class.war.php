...
class war {
	...
	public function setup_form() {
		...
		$player_count = ( isset( $_POST['players'] ) && count( $_POST['players'] ) > 0 ? count( $_POST['players'] ) : 1 );
		$errors = array();
		if ( isset( $_POST['submit'] ) ) {
			...
			foreach( array('face_down_cards'=>'Face down cards'/*,'use_jokers'=>'Use Jokers'*/) as $key => $label ) {
				...
				elseif ( !array_key_exists( $_POST[$key],${$key} ) ) {
			...
			if ( count( $errors ) == 0 ) {
				...
				$this->config['face_down_cards'] = (int) $_POST['face_down_cards'];
				//$this->config['use_jokers']      = ( $_POST['use_jokers'] == 'yes' ? true : false );
	...
	public function play() {
		...
		$vars = array();
		$vars['show'] = ( isset( $_GET['show'] ) ? $_GET['show'] : 'battle' );
		switch( $vars['show'] ) {
			case 'battle':
				$battle_id = 1;
				if ( isset( $_GET['id'] ) && is_numeric( $_GET['id'] ) && isset( $this->battles[$_GET['id']] ) ) {
